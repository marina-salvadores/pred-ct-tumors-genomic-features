...
for (f in list.files("~/Documents/corrections/NUEVO/CNV/", pattern = "all_thresholded.by_genes", full.names = TRUE)){
  ...
  if (counter ==1){
    df_no_og = df[! df$`Gene Symbol` %in% c(og299$gene, genes$`Gene Symbol`, gene_list1, gene_list2),]
    a = sample_n(df_no_og, 300)
    gene_list = a$`Gene Symbol`
  }
  
  df_red1 = df[df$`Gene Symbol` %in% gene_list,]
  
  df_red2 = data.frame(t(df_red1[, 4:ncol(df_red1)]))
  colnames(df_red2) = gene_list
...
# write file
write.csv(final_cna, '../../results/CNAs/CNA_realWXS_300random_genes3.csv', row.names = FALSE)
